...
def li_login():
    # Skip real redirect, just pretend login works
    return redirect(url_for("li_callback"))

@app.route("/oauth/linkedin/callback")
def li_callback():
    # Pretend login succeeded
    session["li_token"] = "demo_token"
    flash("✅ LinkedIn login successful (demo)", "success")
    return redirect(url_for("li_collect"))  # 👈 go straight to LinkedIn tab

@app.route("/linkedin/collect", methods=["GET", "POST"])
def li_collect():
    if "li_token" not in session:
        flash("❌ Please log in with LinkedIn first.", "error")
        return redirect(url_for("index", active_tab="linkedin"))

    # Add compliance flagging to mock posts
    flagged_posts = []
    for p in mock_li_posts:
        violations = check_compliance(p["message"])
...
def li_create():
    ...
    if msg:
        ...
        if violations:
            flash(f"⚠️ Compliance violation detected: {', '.join(violations)}", "error")
        else:
            flash("✅ LinkedIn post published (demo)", "success")
    else:
        flash("⚠️ Empty post not published.", "error")
...
def tw_login():
    # Mock login flow (pretend success)
    session["tw_token"] = "mock_twitter_token"
    flash("✅ Twitter login successful (mock)", "success")

    # Immediately fetch mock posts and show them
    posts = [
        ...
        {"id": "t3", "message": "Our goal is to help you identify potentially suitable investment options aimed to help accomodate your financial objectives.", "created_time": "Sep 02, 2025 11:00 AM"},
    ]

    for p in posts:
        p["violations"] = check_compliance(p["message"])

    return render_template("index.html", tw_posts=posts, active_tab="twitter")
...
def tw_create():
    ...
    mock_post = {"id": "mock_new", "message": msg, "created_time": format_date(str(datetime.utcnow()))}
    mock_post["violations"] = check_compliance(msg)

    flash("✅ Mock Twitter post created", "success")
    return render_template("index.html", tw_posts=[mock_post], active_tab="twitter")
...
def tw_delete():
    ...
    flash("✅ Mock Twitter post deleted", "success")
    # Just refresh with no posts for simplicity
    return render_template("index.html", tw_posts=[], active_tab="twitter")
